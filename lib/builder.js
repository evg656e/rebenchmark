...
function resetReporter(newReporter, printPlatform) {
    reporter = newReporter;
    reporter.setup(printPlatform && platform ? platform : undefined);
}

export function resetOptions({ grep, reporter, reporterOptions, platform, ...opts } = {}) {
    options = {
        ...opts,
        grep: coerceInclude(grep),
        reporter: coerceReporter(reporter, reporterOptions)
    };
    resetReporter(options.reporter, platform);
    return emitter;
}
